...
void BaseFigure::setType(const FigureType &_type)
{
    if (_type < 0 || _type >= TYPES_SZ) {
        throw invalid_argument("invalid argument: out of range in FigureType value");
        return;
    }
...
string BaseFigure::getName()
{
    return getProperty(NAME_PROP);
}

void BaseFigure::setName(const string &_name)
{
    addProperty(NAME_PROP, _name);
}

void BaseFigure::addInProperties(const std::string &_propName, const std::string &_prop)
{
